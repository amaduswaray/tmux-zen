...
# Check if enabled
ENABLED=$(tmux show-option -gv @tmux_show_netspeed 2>/dev/null)
[[ ${ENABLED} -ne 1 ]] && exit 0
...
OUTPUT="#[bg=default]${RESET} #[bg=default]${NET_ICONS[traffic_rx]} #[bg=default]$RX_SPEED #[bg=default]${NET_ICONS[traffic_tx]} #[bg=default]$TX_SPEED #[bg=default]$NETWORK_ICON #[bg=default]$INTERFACE"
if [[ ${SHOW_IP} -ne 0 ]] && [[ -n $IPV4_ADDR ]]; then
  OUTPUT+="#[bg=default]${NET_ICONS[ip]} #[bg=default]$IPV4_ADDR "
fi
